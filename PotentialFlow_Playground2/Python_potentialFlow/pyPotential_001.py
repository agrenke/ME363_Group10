...
import os
import numpy
from scipy import integrate, linalg, math
from matplotlib import pyplot


# load geometry from data file
naca_filepath = os.path.join('x.dat')
with open(naca_filepath, 'r') as infile:
    x = numpy.loadtxt(infile, dtype=float, unpack=True)

naca_filepath = os.path.join('y.dat')
with open(naca_filepath, 'r') as infile:
    y = numpy.loadtxt(infile, dtype=float, unpack=True)
...
class Freestream:
    ...
    """
    def __init__(self, u_inf=1.0, alpha=10):
        """
...
freestream = Freestream(u_inf=1.0, alpha=10)
...
def get_velocity_field(panels, freestream, X, Y):
    ...
    for panel in panels:
        u += panel.sigma / (2.0 * numpy.pi) * vec_intregral(X, Y, panel, 1.0, 0.0)
        v += panel.sigma / (2.0 * numpy.pi) * vec_intregral(X, Y, panel, 0.0, 1.0)
...
x_start, x_end = -150, 150
y_start, y_end = -150, 150
...
pyplot.axis('scaled', adjustable='box')
pyplot.title('Streamlines around a NACA 0012 airfoil (AoA = ${}^o$)'.format(freestream.alpha),
             fontsize=16);
pyplot.show()
